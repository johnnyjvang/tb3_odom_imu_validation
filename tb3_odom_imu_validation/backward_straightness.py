...
"""
backward_straightness.py

Drive the robot backward using odometry distance and evaluate how straight
the motion was using:
- odometry lateral drift
- IMU heading change during the backward motion

Goal:
- Confirm the robot can drive backward in a mostly straight line
...
LINEAR_SPEED = -0.08           # m/s, negative for backward
...
class BackwardStraightness(Node):
    def __init__(self):
        super().__init__('backward_straightness')
        ...
        self.get_logger().info('Starting backward_straightness test')
    ...
    def finish_and_exit(self):
        ...
        if self.start_x is None or self.start_y is None or self.start_imu_yaw is None:
            ...
            self.get_logger().info('=== Backward Straightness Results ===')
            self.get_logger().info(f'Backward distance traveled: {distance:.3f} m')
    ...
    def loop(self):
        if self.done:
            if time.time() - self.finish_time > 0.5:
                self.get_logger().info('Exiting backward_straightness')
                rclpy.shutdown()
            return
        ...
        if self.phase == 'driving':
            ...
            if distance >= TARGET_DISTANCE:
                self.get_logger().info('Target backward distance reached')
                self.phase = 'finished'
